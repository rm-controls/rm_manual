...
{
  // sub
  data_.joint_state_sub_ =
      nh.subscribe<sensor_msgs::JointState>("/joint_states", 10, &ManualBase::jointStateCallback, this);
  data_.actuator_state_sub_ =
      nh.subscribe<rm_msgs::ActuatorState>("/actuator_states", 10, &ManualBase::actuatorStateCallback, this);
  data_.dbus_sub_ = nh.subscribe<rm_msgs::DbusData>("/dbus_data", 10, &ManualBase::dbusDataCallback, this);
  data_.track_sub_ = nh.subscribe<rm_msgs::TrackData>("/track", 10, &ManualBase::trackCallback, this);
  data_.gimbal_des_error_sub_ = nh.subscribe<rm_msgs::GimbalDesError>("/controllers/gimbal_controller/error", 10,
                                                                      &ManualBase::gimbalDesErrorCallback, this);
  data_.odom_sub_ = nh.subscribe<nav_msgs::Odometry>("/odom", 10, &ManualBase::odomCallback, this);
  data_.game_robot_status_sub_ =
      nh.subscribe<rm_msgs::GameRobotStatus>("/game_robot_status", 10, &ManualBase::gameRobotStatusCallback, this);
  data_.game_robot_hp_sub_ =
      nh.subscribe<rm_msgs::GameRobotHp>("/game_robot_hp", 10, &ManualBase::gameRobotHpCallback, this);
  data_.game_status_sub_ = nh.subscribe<rm_msgs::GameStatus>("/game_status", 10, &ManualBase::gameStatusCallback, this);
  data_.capacity_sub_ =
      nh.subscribe<rm_msgs::CapacityData>("/capacity_data", 10, &ManualBase::capacityDataCallback, this);
  data_.power_heat_data_sub_ =
      nh.subscribe<rm_msgs::PowerHeatData>("/power_heat_data", 10, &ManualBase::powerHeatDataCallback, this);
  data_.referee_sub_ = nh.subscribe<rm_msgs::Referee>("/referee", 10, &ManualBase::refereeCallback, this);
  // pub
  data_.manual_to_referee_pub_ = nh.advertise<rm_msgs::ManualToReferee>("/manual_to_referee", 1);
...
{
  robot_hp_event_.update(data_.game_robot_status_data_.remain_hp != 0);
}
